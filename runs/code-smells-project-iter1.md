# Run code-smells-project/iter1 — refactor-arch on code-smells-project

Invocation: `/refactor-arch code-smells-project --yes` (sub-agent run, fresh context; confirmation
auto-approved because no human could answer the gate). Date: 2026-09-21.
Baseline/replay port: 5101 (Flask CLI flag, not written into the project).
...
Language:      Python (100% of authored source; host runtime Python 3.13.2)
Framework:     Flask 3.1.1 (exact pin in requirements.txt; installed metadata in the validation venv), flask-cors 5.0.1
Dependencies:  flask==3.1.1, flask-cors==5.0.1 (runtime; no dev dependencies; no lockfile — pins are exact)
Domain:        E-commerce API for a store ("loja"): products, users/login, orders with stock control, sales report
App type:      HTTP service (JSON API) — 19 routes: 17 via add_url_rule in app.py + 2 decorated admin routes + index
Architecture:  None/nominal — 4 flat modules: app.py (config + routing + raw-SQL admin handlers), controllers.py (handlers with validation, rules, notifications, direct SQL), models.py (SQL + business rules + serialization), database.py (global connection + DDL + seed)
Source files:  4 files analyzed (*.py authored in the target; excluded: README.md, requirements.txt, .gitignore'd *.db/__pycache__)
DB tables:     produtos, usuarios, pedidos, itens_pedido (SQLite file loja.db, DDL in database.py)
================================
Boot command:  python -m flask --app <target>/app.py run --port 5101   (cwd: scratch dir, fresh loja.db)
               (the project's own `python app.py` hardcodes port 5000 in __main__)
```

Note: Flask is not installed in the host interpreter; the pinned requirements were installed into a
throwaway venv outside the target. No target file was written in Phase 1.

---

## Phase 2 report + Phase 3 addendum (content of code-smells-project/reports/audit-latest.md)
